...
class Charts extends Component {
    ...
    initializeActivityLog = () => {
        ...
        console.log(
            'Time:',
            moment(this.props.user_activities[1].updatedAt).format('M'),
        );
    ...
    getNumActivities = (timescale, date) => {
        ...
        this.props.user_activities.forEach(activity => {
            console.log(moment(activity.updatedAt).format('D'), date);
        });
        if (timescale === 'Day') {
            return this.props.user_activities.filter(
                activity => moment(activity.updatedAt).format('D') === date,
            ).length;
        } else if (timescale === 'Month') {
            return this.props.user_activities.filter(
                activity => moment(activity.updatedAt).format('M') === date,
            ).length;
        }
    };
